...
# from model.TransLite3D import TransLite3D
...
model=UNETR(in_channels=1,out_channels=1,img_size=(128,192,192)).cuda()
model.load_state_dict(pt.load(model_path+'/DSRL/UNETR_3D_BraTS_patch-free_bs1_best.pt',map_location = 'cpu'))
...
for x in range(epoch):
    ...
    if dice>best_dice:
        best_dice=dice
        print('New best dice! Model saved to',model_path+'/DSRL/UNETR_3D_BraTS_patch-free_bs'+str(batch_size)+'_best.pt')
        pt.save(model.state_dict(), model_path+'/DSRL/UNETR_3D_BraTS_patch-free_bs'+str(batch_size)+'_best.pt')
    # print('===TEST===>')
    # TestModel() 
