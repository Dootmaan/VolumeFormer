import torch as pt
import numpy as np
from model.vision_transformer import VTUNet
...
model=VTUNet().cuda()
model.load_state_dict(pt.load(model_path+'/VT-UNet_Large_3D_BraTS_patch-free_bs1_best.pt',map_location = 'cpu'))
...
for x in range(epoch):
    ...
    if dice>best_dice:
        best_dice=dice
        print('New best dice! Model saved to',model_path+'/VTUNet_3D_BraTS_patch-free_bs'+str(batch_size)+'_best.pt')
        pt.save(model.state_dict(), model_path+'/VTUNet_3D_BraTS_patch-free_bs'+str(batch_size)+'_best.pt')
    # print('===TEST===>')
    # TestModel() 
