import torch as pt
import numpy as np
from model.TransLite3D import TransLite3D_ras_hd_noformer
...
model=TransLite3D_ras_hd_noformer(in_channel=1,inter_channel=4,out_channel=1).cuda()
model.load_state_dict(pt.load(model_path+'/TransLite_3D_hd_noformer_BraTS_patch-free_bs1_best.pt',map_location = 'cpu'))
...
for x in range(epoch):
    ...
    for i,data in enumerate(train_dataset):
        ...
        if i%10==0:
            ...
            cv2.imwrite('TransLite_3d_patchfree_combine.png',final_img)
    ...
    if dice>best_dice:
        best_dice=dice
        print('New best dice! Model saved to',model_path+'/TransLite_3D_hd_noformer_BraTS_patch-free_bs'+str(batch_size)+'_best.pt')
        pt.save(model.state_dict(), model_path+'/TransLite_3D_hd_noformer_BraTS_patch-free_bs'+str(batch_size)+'_best.pt')
    # print('===TEST===>')
    # TestModel() 
