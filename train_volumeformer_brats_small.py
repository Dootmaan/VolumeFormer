import torch as pt
import numpy as np
from model.VolumeFormer import VolumeFormer_ras_small
...
model=VolumeFormer_ras_small(1,4,1).cuda()
model.load_state_dict(pt.load(model_path+'/VolumeFormer_ras_small_BraTS_patch-free_bs1_best.pt',map_location = 'cpu'))
...
for x in range(epoch):
    ...
    if dice>best_dice:
        best_dice=dice
        print('New best dice! Model saved to',model_path+'/VolumeFormer_ras_small_BraTS_patch-free_bs'+str(batch_size)+'_best.pt')
        pt.save(model.state_dict(), model_path+'/VolumeFormer_ras_small_BraTS_patch-free_bs'+str(batch_size)+'_best.pt')
    # print('===TEST===>')
    # TestModel() 
