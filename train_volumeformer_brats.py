...
import torch as pt
import numpy as np
from model.VolumeFormer import VolumeFormer_ras
...
model=VolumeFormer_ras(in_channel=1,inter_channel=4,out_channel=1).cuda()
# model.load_state_dict(pt.load(model_path+'/VolumeFormer_3D_ras_vskip_groups_BraTS_patch-free_bs1_best.pt',map_location = 'cpu'))
...
for x in range(epoch):
    ...
    if dice>best_dice:
        best_dice=dice
        print('New best dice! Model saved to',model_path+'/VolumeFormer_3D_ras_vskip_groups_BraTS_patch-free_bs'+str(batch_size)+'_best.pt')
        pt.save(model.state_dict(), model_path+'/VolumeFormer_3D_ras_vskip_groups_BraTS_patch-free_bs'+str(batch_size)+'_best.pt')
    # print('===TEST===>')
    # TestModel() 
